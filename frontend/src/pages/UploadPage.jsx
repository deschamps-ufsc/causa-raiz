...
export default function UploadPage() {
  ...
  // Permite navegar para uma aba via state da rota
  ...
  useEffect(() => {
    if (usinaAtual && activeTab === 'infos') {
      fetchUsinaInfo(usinaAtual).then(setInfoData).catch(() => {})
    }
  }, [usinaAtual, activeTab])

  const handleInfoImport = async () => {
    if (!infoFile || !usinaAtual) return
    setInfoLoading(true); setInfoError(null)
    try {
      const res = await importUsinaInfo(usinaAtual, infoFile)
      setInfoResult(res)
      const updated = await fetchUsinaInfo(usinaAtual)
      setInfoData(updated)
  ...
  const [synthData, setSynthData] = useState({})  // { batch_id: { nome, formula, series[] } }
  const [synthDragging, setSynthDragging] = useState(false)
  const [batchEdits, setBatchEdits] = useState({}) // { batch_id: { formula, nome } }
  const [batchSaving, setBatchSaving] = useState({})
  const synthInputRef = useRef()

  const reloadSynths = (usina) =>
    fetchSynthetics(usina).then(d => {
      setSynthData(d)
      const edits = {}
  ...
  useEffect(() => {
    if (usinaAtual && activeTab === 'sintetica') reloadSynths(usinaAtual)
  }, [usinaAtual, activeTab])

  const handleSynthImport = async () => {
    if (!synthFile || !usinaAtual) return
    setSynthLoading(true); setSynthError(null)
    try {
      const res = await importSyntheticExcel(usinaAtual, synthFile, synthNomeGrupo)
      setSynthResult(res)
  ...
  const handleBatchSave = async (batchId) => {
    ...
    try {
      ...
      await saveBatchConfig(usinaAtual, batchId, formula || '', nome || '')
      await reloadSynths(usinaAtual)
    } catch (e) { setSynthError(e.message) }
  ...
  if (!usinaAtual) {
    return (
      <div style={{ minHeight: 'calc(100vh - 60px)', background: 'var(--bg-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 16, color: 'var(--text-secondary)', textAlign: 'center' }}>
  ...
  return (
    <div style={{ minHeight: 'calc(100vh - 60px)', background: 'var(--bg-primary)', padding: '24px' }}>
      <div style={{ maxWidth: 760, margin: '0 auto' }}>
        {/* Header */}
        <div style={{ textAlign: 'center', marginBottom: 24 }}>
          <h1 style={{ fontSize: 22, fontWeight: 800, letterSpacing: -0.5, marginBottom: 4 }}>Upload & Configuração</h1>
          <p style={{ color: 'var(--text-secondary)', fontSize: 13 }}>
            Usina: <strong style={{ color: 'var(--amber)' }}>{usinaAtual}</strong>
          </p>
        </div>
                ...
                <div style={{ overflowX: 'auto' }}>
                      ...
                      {Object.entries(infoData).slice((infoPage - 1) * 20, infoPage * 20).map(([serie, row], i) => {
                        ...
                        return (
                          <tr key={i} style={{ background: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.02)' }}>
                            <td style={tdStyle}>{skid}</td>
                            <td style={tdStyle}>{inv}</td>
                            <td style={tdStyle}>{sb}</td>
                  ...
                  {Object.keys(infoData).length > 20 && (
                    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 10, padding: '12px 10px', fontSize: 13, color: 'var(--text-secondary)' }}>
                      <button 
                        onClick={() => setInfoPage(p => Math.max(1, p - 10))} 
                        disabled={infoPage <= 10}
                        style={{ padding: '6px 10px', fontSize: 12, borderRadius: 6, border: '1px solid var(--border)', background: infoPage <= 10 ? 'transparent' : 'var(--bg-secondary)', color: infoPage <= 10 ? 'var(--text-muted)' : 'var(--text-primary)', cursor: infoPage <= 10 ? 'not-allowed' : 'pointer' }}
                      ...
                      <button 
                        ...
                        disabled={infoPage === 1}
                        style={{ padding: '6px 14px', fontSize: 12, borderRadius: 6, border: '1px solid var(--border)', background: infoPage === 1 ? 'transparent' : 'var(--bg-secondary)', color: infoPage === 1 ? 'var(--text-muted)' : 'var(--text-primary)', cursor: infoPage === 1 ? 'not-allowed' : 'pointer' }}
                      >
                        Anterior
                      </button>
                      
                      <span style={{ fontWeight: 500, margin: '0 8px' }}>
                        Página {infoPage} de {Math.ceil(Object.keys(infoData).length / 20)}
                      </span>
                      
                      <button 
                        onClick={() => setInfoPage(p => Math.min(Math.ceil(Object.keys(infoData).length / 20), p + 1))} 
                        disabled={infoPage >= Math.ceil(Object.keys(infoData).length / 20)}
                        style={{ padding: '6px 14px', fontSize: 12, borderRadius: 6, border: '1px solid var(--border)', background: infoPage >= Math.ceil(Object.keys(infoData).length / 20) ? 'transparent' : 'var(--bg-secondary)', color: infoPage >= Math.ceil(Object.keys(infoData).length / 20) ? 'var(--text-muted)' : 'var(--text-primary)', cursor: infoPage >= Math.ceil(Object.keys(infoData).length / 20) ? 'not-allowed' : 'pointer' }}
                      >
                        Próxima
                      </button>

                      <button 
                        onClick={() => setInfoPage(p => Math.min(Math.ceil(Object.keys(infoData).length / 20), p + 10))} 
                        disabled={infoPage > Math.ceil(Object.keys(infoData).length / 20) - 10}
                        style={{ padding: '6px 10px', fontSize: 12, borderRadius: 6, border: '1px solid var(--border)', background: infoPage > Math.ceil(Object.keys(infoData).length / 20) - 10 ? 'transparent' : 'var(--bg-secondary)', color: infoPage > Math.ceil(Object.keys(infoData).length / 20) - 10 ? 'var(--text-muted)' : 'var(--text-primary)', cursor: infoPage > Math.ceil(Object.keys(infoData).length / 20) - 10 ? 'not-allowed' : 'pointer' }}
            ...
            <div className="card" style={{ marginBottom: 16 }}>
              ...
              <input
                ...
                style={{
                  ...
                  background: 'var(--bg-input, #1e293b)', border: '1px solid var(--border)',
              ...
              <div
                ...
                style={{
                  border: `2px dashed ${synthDragging ? 'var(--amber)' : synthFile ? 'var(--green)' : 'var(--border)'}`,
                  background: synthDragging ? 'var(--amber-glow)' : 'transparent',
                  cursor: 'pointer', textAlign: 'center', padding: '24px 20px',
                  borderRadius: 8, transition: 'all 0.2s', marginBottom: 12,
                }}
              >
                  ...
                  {Object.entries(synthData).map(([batchId, batch]) => {
                    ...
                    return (
                      <div key={batchId} style={{
                        ...
                        padding: '14px 16px', border: `1px solid ${batch.formula ? 'var(--green, #22c55e)33' : 'var(--amber)44'}`,
                      }}>
                        {/* Header do grupo */}
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            ...
                            <input
                              value={edit.nome}
                              onChange={e => setBatchEdits(p => ({ ...p, [batchId]: { ...p[batchId], nome: e.target.value } }))}
                              style={{
                                background: 'transparent', border: 'none', borderBottom: '1px solid var(--border)',
                                color: 'var(--text-primary)', fontWeight: 700, fontSize: 14, padding: '2px 4px', minWidth: 180,
                              }}
                            />
                          </div>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                            <span style={{ fontSize: 11, color: 'var(--text-muted)', background: 'var(--bg-primary)', borderRadius: 20, padding: '2px 8px' }}>
                              {totalSeries} séries
                            </span>
                            <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                              {new Date(batch.criado_em).toLocaleDateString('pt-BR')}
                            </span>
                            <button
                              onClick={() => handleBatchDelete(batchId, batch.nome)}
                        ...
                        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                          ...
                          <input
                            value={edit.formula}
                            onChange={e => setBatchEdits(p => ({ ...p, [batchId]: { ...p[batchId], formula: e.target.value } }))}
                            placeholder="ex: S1 * S2   ou   (S1 + S2) / 2"
                            style={{
                              flex: 1, background: 'var(--bg-input, #1e293b)', border: '1px solid var(--border)',
                              ...
                              {batch.series.slice(0, 50).map((s, i) => (
                                <div key={i} style={{ padding: '2px 0', borderBottom: '1px solid var(--border)22' }}>
                                  ...
                                  <span style={{ color: 'var(--text-muted)' }}>
                                    [{s.serie_1}{s.serie_2 ? `, ${s.serie_2}` : ''}{s.serie_3 ? `, ${s.serie_3}` : ''}]
                                  </span>
                                </div>
                              ))}
